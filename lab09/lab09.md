...
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - Потому что ICMP -- протокол сетевого уровня (ниже транспортного).
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   ...
   - Checksum, Identifier, Sequence Number, Timestamp, Data
   - 2, 2, 2.
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - Все поля совпадают, кроме Timestamp -- это поле отсутствует.
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - Заголовок IP датаграммы эхо-запроса. Также есть поле Unused: 00000000 (Пока в нем нет полезной информации, оно зарезервировано под будущие расширения).
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - Тип этих пакетов 0 (Echo reply), есть поля Identifier и Sequence Number. Это точь-в-точб пакет, полученный в задании №1 (то есть корректный Echo ответ).
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   ![](./images/traceroute_3.png)
   
   - Довольно сложно ответить на вопрос о среднем значении задержки, бОльшая часть маршрутизаторов не отвечают на ICMP пакеты (2 -- 8).
   - С помощью специализированного сервиса можно узнать локацию конечного хоста (конечный хост импользует AWS -- amazon web services) -- Америка. 

   ![](./images/aws_location.png)
