...
3. Каковы значения Transaction-ID в наборе (Request/ACK) DHCP-сообщений? Опишите
   ...
   Transaction-ID нужен для идентификации DHCP-транзакции, по этому полю можно легко понять, к какому запросу относится ответ DHCP сервера.
...
4. Хост использует DHCP-протокол, в частности, для получения IP-адреса. Но IP-адрес хоста
   ...
   Offer. Source: `192.168.0.1`, Destination: `192.168.0.3`

   - ![](./images/wireshark_5.png)
   Request. Source: `0.0.0.0`, Destination: `255.255.255.255`

   - ![](./images/wireshark_6.png)
   ACK. Source: `192.168.0.1`, Destination: `192.168.0.3`
...
6. Объясните назначение срока аренды IP-адреса. Какова длительность срока аренды в
   ...
   1 день. Срок аренды нужен для перераспределения IP адресов в сети.
